...
class yolov11_heatmap:
    def __init__(self, weight, cfg, device, method, layer, backward_type, conf_threshold, ratio):
        ...
        self.model_names = self.ckpt['model'].names
        self.csd = self.ckpt['model'].float().state_dict()  # checkpoint state_dict as FP32
        self.model = model = Model(cfg, ch=3, nc=len(self.model_names)).to(device)
        self.csd = intersect_dicts(self.csd, self.model.state_dict(), exclude=['anchor'])  # intersect
        self.model.load_state_dict(self.csd, strict=False)  # load
        self.model.eval()
        print(f'Transferred {len(self.csd)}/{len(self.model.state_dict())} items')
    ...
    def __call__(self, img_path, save_path):
        ...
        for i in trange(int(post_result.size(0) * self.ratio)):
            ...
            cam_image = show_cam_on_image(img.copy(), saliency_map, use_rgb=True, image_weight=0.6)
...
def get_params():
    ...
    for grad_method in grad_list:
        params = {
            'weight': r'E:\Git\ultralytics\runs\detect\HYJTFJW_coal_det\0829_e150_i320_b8_cfg1\weights\best.pt',  # 训练出来的权重文件
            'cfg': r'E:\Git\ultralytics\ultralytics\cfg\models\11\yolo11.yaml',  # 训练权重对应的yaml配置文件
            'device': 'cuda:0',
            'method': grad_method,
